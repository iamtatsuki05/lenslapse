...
def probe_cfg(**kw: Any) -> ProbeCliConfig:
    base = dict(model="m", text="t", step=None, pos=None, targets=None, json_output=False)
    return ProbeCliConfig(**{**base, **kw})


def trace_cfg(**kw: Any) -> TraceCliConfig:
    base = dict(model="m", text="t", pos=None, layer=None, top=3, targets=None, json_output=False)
    return TraceCliConfig(**{**base, **kw})
...
def test_probe_step_validation_depends_on_mode(caplog: pytest.LogCaptureFixture) -> None:
    with pytest.raises(SystemExit, match="no checkpoint for step 5"):  # local/final steps are exhaustive
        cmd_probe(FakeBackend([0, 100], mode="local"), probe_cfg(step=5))
    suite = FakeBackend([0, 100], mode="suite")  # a suite grid is a subset of the hub's revisions
    with caplog.at_level("INFO", logger="lenslapse.client"):
        cmd_probe(suite, probe_cfg(step=5))
...
def test_probe_json_prints_the_exact_payload(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    cmd_probe(backend, probe_cfg(step=10, targets="22", json_output=True))
...
def test_trace_needs_a_suite() -> None:
    with pytest.raises(SystemExit, match="needs a suite"):
        cmd_trace(FakeBackend([0]), trace_cfg())


def test_trace_rejects_out_of_range_pos() -> None:
    with pytest.raises(SystemExit, match="--pos must be"):
        cmd_trace(FakeBackend([0, 10]), trace_cfg(pos=5))


def test_trace_rejects_bad_top() -> None:
    with pytest.raises(SystemExit, match="--top must be"):
        cmd_trace(FakeBackend([0, 10]), trace_cfg(top=0))


def test_bare_models_call_routes_to_list(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    picked: list[ConnectionConfig] = []

    def fake_choose(conn: ConnectionConfig) -> FakeBackend:
        picked.append(conn)
        return FakeBackend([0, 10])

    monkeypatch.setattr(client, "choose_backend", fake_choose)
    # `lenslapse models` with no further subcommand (ModelsCommands.__call__) must behave like `list`
    ModelsCommands()()
...
def test_choose_backend_auto_detects_then_falls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert isinstance(client.choose_backend(ConnectionConfig()), HttpBackend)

    monkeypatch.setattr(client, "server_alive", lambda base: False)
    assert client.choose_backend(ConnectionConfig()).where == "in-process"

    monkeypatch.setattr(client, "server_alive", lambda base: pytest.fail("--local must not even look for a server"))
    assert client.choose_backend(ConnectionConfig(local=True)).where == "in-process"

    with pytest.raises(SystemExit, match="no probe server responding"):
        monkeypatch.setattr(client, "server_alive", lambda base: False)
        client.choose_backend(ConnectionConfig(server="http://localhost:9"))
